...
@register_model('transformer_nat_ymask_pred_len')
class TransformerNatYmaskPredLenModel(FairseqEncoderDecoderModel):
    """
    ...
    def build_encoder(cls, args, src_dict, embed_tokens):
        return TransformerYmaskEncoder(args, src_dict, embed_tokens)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return TransformerYmaskDecoder(args, tgt_dict, embed_tokens)
...
class TransformerYmaskDecoder(FairseqDecoder):
    """
    ...
    def forward(self, prev_output_tokens, encoder_out=None, **unused):
        ...
        decoder_input_embeddings = self.embed_scale * self.embed_tokens(prev_output_tokens)
        targets_padding = prev_output_tokens.eq(self.padding_idx)

        positions = self.embed_positions(
            decoder_input_ids,
        ) if self.embed_positions is not None else None
        x, extra = self.extract_features(decoder_input_embeddings, 
            encoder_out, 
            targets_padding=targets_padding,
            positions=positions)
        x = self.output_layer(x)
        # only work with share_all_embedddings=True
        encoderout_probs = self.output_layer(encoder_out['encoder_out'].transpose(0, 1))
        extra['encoder_out'] = encoderout_probs
        return x, extra, targets_padding

    def extract_features(self, prev_output_tokens, encoder_out=None, targets_padding=None, positions=None, **unused):
        """
        Similar to *forward* but only return features.
        Add by JL: decoder_self_attention_mask: (batch, tgt_len, tgt_len)
        REMOVED, change self.buffered_future_mask(x) can achieve same function
        Returns:
            tuple:
        ...
        for layer in self.layers:
            x, attn = layer(
                ...
                self_attn_mask=self.without_self_mask(x),
    ...
    def buffered_future_mask(self, tensor):
        ...
        output_mask = self._future_mask[:dim, :dim]
        return output_mask
...
@register_model_architecture('transformer_nat_ymask_pred_len', 'transformer_nat_ymask_pred_len')
def base_architecture(args):
    ...
    args.positional_attention = getattr(args, 'positional_attention', True)

@register_model_architecture('transformer_nat_ymask_pred_len', 'transformer_nat_ymask_pred_len_deep_small')
def transformer_deep_small(args):
...
@register_model_architecture('transformer_nat_ymask_pred_len', 'transformer_nat_ymask_pred_len_wmt_en_de')
